...
object StatusBarCompat {
    ...
    private fun setStatusBarColor(activity: Activity, @ColorInt color: Int) {
        ...
        window.statusBarColor = if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M && color == Color.WHITE) Color.rgb(0xBA, 0xBA, 0xBA) else color
        if (childView != null) {
...
//            // if it has already added the status view,we will remove previous one.
//            removeStatusBarViewIfExist(activity);
//            // add the child view whose height is equals to the height of the status bar
